//package com.wangdh.demolist.pos.utils;
//
...
//            throw new IllegalArgumentException(LanguageUtil.getInstanse().getLanguageString("传入的参数长度必须是8的倍数","The incoming parameter length must be a multiple of 8"));
//        }
...
//            throw new IllegalArgumentException(LanguageUtils.getInstanse().getLanguageString("参数非法：offset:"+offset+",length:"+length+",字节长度："+bts.length,"Parameters of illegal：offset:"+offset+",length:"+length+",bytes："+bts.length));
//        }
...
//            throw new IllegalArgumentException(LanguageUtils.getInstanse().getLanguageString("参数length不能小于0,length:"+length,"The parameter length cannot be less than 0,length:"+length));
//        }else if(length==0){
//            return new byte[0];
//        }
